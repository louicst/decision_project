...
def questions_8_to_10_diverse_lorenz_policies(mdp, step=0.1):
    """
    Questions 8 & 10: Generates unique Pareto policies via weighted sums,
    then filters them to display the final Lorenz-efficient set.
    """
    ...
    for w in weights_list:
        ...
        if pol_tuple not in seen_policies:
            ...
            # Extract raw values for reporting and normalized values for math
    ...
    # --- 2. FILTERING PHASE (Lorenz Dominance) ---
    for i, p1 in enumerate(unique_policies):
        L1 = p1['l_vec']
        for j, p2 in enumerate(unique_policies):
            if i == j: continue
            L2 = p2['l_vec']
            # Dominance check: L2 >= L1 everywhere and L2 > L1 somewhere
            if np.all(L2 >= L1) and np.any(L2 > L1):
                p1['lorenz_efficient'] = False
                break 

    # --- 3. DISPLAY TABLE FOR QUESTION 8 ---
    print("\n" + "="*95)
    print(" QUESTION 8: Table of all unique Pareto-efficient policies generated by scalarization")
    print("="*95)
    print(f"{'Policy (Actions for States 1-10)':<35} | {'Profit (€)':<12} | {'Carbon':<10} | {'Bio':<8}")
    print("-" * 95)
    for p in unique_policies:
        pol_str = str(p['policy'])
        print(f"{pol_str:<35} | {p['v_raw'][0]:<12.2f} | {p['v_raw'][1]:<10.2f} | {p['v_raw'][2]:<8.2f}")
    print(f"\n[INFO] Found {len(unique_policies)} unique policies using {len(weights_list)} weight vectors.")


    # --- 4. DISPLAY TABLE FOR QUESTION 10 ---
    print("\n\n" + "="*95)
    print(" QUESTION 10: Final Lorenz-Efficient Policies (Balanced Trade-offs)")
    print("="*95)
    print(f"{'Policy (Actions for States 1-10)':<35} | {'Profit (€)':<12} | {'Carbon':<10} | {'Bio':<8}")
    print("-" * 95)
    ...
    lorenz_count = 0
    for p in unique_policies:
        if p['lorenz_efficient']:
            lorenz_count += 1
            pol_str = str(p['policy'])
            print(f"{pol_str:<35} | {p['v_raw'][0]:<12.2f} | {p['v_raw'][1]:<10.2f} | {p['v_raw'][2]:<8.2f}")
            
    print(f"\n[OK] Filtered down to {lorenz_count} Lorenz-efficient policies.")
    print("="*95 + "\n")
